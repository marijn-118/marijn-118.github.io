...
# Load PULocationID instead of DOLocationID
...
# Calculate the average tip for EVERY pickup zone
zone_tips = taxi_df.groupby('PULocationID')['tip_percent'].mean().reset_index()

print("5. Merging Danger Scores with Tip Averages...")
# Merge on PULocationID
merged_df = pd.merge(zone_tips, danger_df, on='PULocationID', how='inner')
...
plt.title('Correlation: Neighborhood Danger vs. Average Tip % (Pickups)', fontsize=14, pad=15)
plt.xlabel('Total Crimes in Pickup Zone (2023)', fontsize=12)
...
plt.tight_layout()
plt.savefig('Scatter_Danger_Tip_Pickups.png', dpi=300)
plt.show()
