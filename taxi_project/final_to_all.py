...
# CRITICAL: We added DOLocationID (Drop-Off Location) to the list
...
print("4. Merging the Datasets (Testing Drop-offs)...")
# CRITICAL FIX: We rename the Danger Dataframe column so it matches the Drop-off location!
danger_df.rename(columns={'PULocationID': 'DOLocationID'}, inplace=True)

# Now we merge based on where the ride ENDS
merged_df = pd.merge(taxi_df, danger_df, on='DOLocationID', how='inner')
...
# Updated Titles and Labels to reflect the Drop-off pivot
plt.title('Do New Yorkers Tip More When Dropped Off in High-Crime Areas?', fontsize=14, pad=15)
plt.xlabel('Drop-Off Neighborhood Danger Level', fontsize=12)
...
plt.tight_layout()
plt.savefig('Final_Dropoff_Danger_Tip_Plot_To.png', dpi=300)
plt.show()
